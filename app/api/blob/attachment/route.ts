...
import { uploadAttachmentToVercelBlob } from "@/lib/cgmp/vercel-blob-store-server";
...
export async function POST(request: Request) {
  try {
    ...
    const result = await uploadAttachmentToVercelBlob({
      recordId,
      attachment,
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "BLOB_ATTACHMENT_UPLOAD_FAILED",
